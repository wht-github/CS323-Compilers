...
ExtDef:
    Specifier ExtDecList SEMI {
      ...
      $$->push($2);
      $$->push(new Terminal(*$3));
      $$->set_attr(Attr::GLOBALVAR);
    }
  | Specifier SEMI {
    $$ = new ExtDef($1, @1.first_line);
    $$->push(new Terminal(*$2));
    $$->set_attr(Attr::GLOBALSTRUCT);
    }
  | Specifier FunDec CompSt {
    $$ = new ExtDef($1, @1.first_line);
    $$->push($2);
    $$->push($3);
    $$->set_attr(Attr::GLOBALFUNC);
    }
  | ExtDecList error {
...
Exp:
  ...
  | LP Exp RP {
      ...
      $$->push($2);
      $$->push(new Terminal(*$3));
      $$->set_attr(Attr::PARENTHESIS);
    }
  | MINUS Exp {
      $$=new Exp(new Terminal(*$1), @1.first_line);
      $$->push($2);
      $$->set_attr(Attr::UNARYMINUS);
    }
  | NOT Exp {
      $$=new Exp(new Terminal(*$1), @1.first_line);
      $$->push($2);
  ...
  | Exp LB Exp RB {
      ...
      $$->push($3);
      $$->push(new Terminal(*$4));
      $$->set_attr(Attr::ARRAYINDEXING);
    }
  | Exp DOT ID {
      $$ = new Exp($1, @1.first_line);
      $$->push(new Terminal(*$2));
      $$->push(new ValId(*$3));
      $$->set_attr(Attr::MEMBER);
    }
  | ID {
      $$ = new Exp(new ValId(*$1), @1.first_line);
      $$->set_attr(Attr::ID);
